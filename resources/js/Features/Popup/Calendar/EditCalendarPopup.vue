...
const hasChanges = computed(() => {
  return selectedProfessor.value !== initialState.selectedProfessor ||
  ...
  >
    <div class="space-y-6">
      <div>
        ...
        <select v-model="selectedProfessor" class="w-full p-3 border rounded-lg bg-white appearance-none pr-10 relative">
          <option v-for="prof in professors" :key="prof.value" :value="prof.value">
            {{ prof.label }}
          </option>
        </select>
      </div>
      ...
      <div class="space-y-3">
        <div class="flex items-center">
          <input type="checkbox" id="evaluation" v-model="evaluation" class="w-5 h-5 mr-3">
          <label for="evaluation" class="text-lg">Évaluation</label>
        </div>
        <div class="space-y-3">
          <div class="flex items-center">
            <input type="checkbox" id="replaced" v-model="replaced" class="w-5 h-5 mr-3">
            <label for="replaced" class="text-lg">Remplacé</label>
          </div>
          <div v-if="replaced" class="ml-8">
            <label class="block text-lg mb-2">Professeur remplaçant</label>
            <select v-model="replacementProfessor" class="w-full p-3 border rounded-lg bg-white appearance-none pr-10 relative">
              <option v-for="prof in professors" :key="prof.value" :value="prof.value"
        ...
        <div class="flex items-center">
          <input type="checkbox" id="neutralized" v-model="neutralized" class="w-5 h-5 mr-3">
